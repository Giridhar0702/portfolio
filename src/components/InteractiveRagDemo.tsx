    ...
    <section className="py-16 relative z-10 overflow-hidden">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div
          ...
          viewport={{ once: true }}
          className="glass-panel rounded-3xl p-6 sm:p-10 border border-blue-500/40 shadow-2xl relative overflow-hidden"
        >
          {/* Top Title Banner */}
            ...
            <div className="flex flex-wrap gap-2">
              {presetQueries.map((q) => (
                <button
                  ...
                  className="px-3.5 py-2 rounded-xl text-xs font-mono text-slate-300 bg-slate-900/90 hover:bg-slate-800 hover:text-white border border-slate-800 hover:border-cyan-500/40 transition-all text-left disabled:opacity-50"
                >
                  💬 {q}
                </button>
              ))}
          ...
          <div className="relative mb-6">
            <input
              ...
              className="w-full pl-4 pr-28 py-3.5 rounded-2xl bg-slate-950 border border-slate-800 text-slate-200 text-sm focus:outline-none focus:border-blue-500 font-mono"
            />
            <button
              onClick={() => executeRagPipeline(query)}
              disabled={isProcessing}
              className="absolute right-2 top-1/2 -translate-y-1/2 px-4 py-2 rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-semibold text-xs transition-all flex items-center gap-1.5 shadow-md disabled:opacity-50"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            {/* Step Logs Console */}
            <div className="md:col-span-5 rounded-2xl bg-slate-950 p-4 border border-slate-800 font-mono text-[11px] min-h-[160px]">
            ...
            <div className="md:col-span-7 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-950 p-5 border border-slate-800 flex flex-col justify-between">
              <div>
                <div className="text-xs font-mono text-purple-400 mb-2 flex items-center gap-1.5">
                  <Bot className="w-4 h-4 text-purple-400" /> Synthesized Response:
                </div>
                {response ? (
